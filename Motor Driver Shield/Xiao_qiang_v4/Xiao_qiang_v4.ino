...
#define left_rotation_pi_const 0.8
...
double m1_P = 1.09, m1_I = 0.0001, m1_D = 0.001;
  ...
  switch(command){
    case 0:  // idle state, send readings once and check if there is next command
      ...
      if(sendSensorReading){
        ...
        // if obstacle position is x1x1x or x2x2x, reposition robot
        if((obstaclePositions[1]==1 && obstaclePositions[3]==1)||(obstaclePositions[1]==2 && obstaclePositions[3]==2))
...
void obstacleIdentification(){
  ...
  if(leftDiagSensorMedian>=18 && leftDiagSensorMedian<=25)
  ...
  else if(rightSideSensorMedian>=12 && rightSideSensorMedian<=20)
